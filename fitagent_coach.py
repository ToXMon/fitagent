...
# Enhanced Venice AI client
from venice_ai_client import enhanced_venice_client, CoachingStyle, GoalStatus
...
class NutritionResponse(Model):
    ...
    behavior_insights: Optional[str] = Field(default=None, description="Behavioral insights and patterns")
    conversation_id: Optional[str] = Field(default=None, description="Conversation ID for context")
    coaching_style: Optional[str] = Field(default=None, description="Applied coaching style")
...
async def handle_nutrition_query(ctx: Context, sender: str, msg: NutritionQuery):
    """Handle nutrition coaching queries with enhanced Venice AI client"""
    
    try:
        ctx.logger.info(f"Processing enhanced nutrition query for user {msg.user_id}")
        
        # Use enhanced Venice AI client for personalized coaching
        response_data = await enhanced_venice_client.personalized_coaching_response(
            user_id=msg.user_id,
            query=msg.query,
        ...
        nutrition_response = NutritionResponse(
            ...
            behavior_insights=response_data.get("behavior_insights", "Keep building healthy habits!"),
            conversation_id=msg.conversation_id,
            coaching_style=response_data.get("coaching_style", "motivational")
        )
        
        # Update user goals if provided
        if msg.user_goals:
            for goal_type, target_value in msg.user_goals.items():
                if isinstance(target_value, (int, float)):
                    await enhanced_venice_client.update_goal(
                        msg.user_id, goal_type, target_value, 0
                    )
        
        ctx.logger.info(f"Sending enhanced nutrition response to {sender}")
...
# Enhanced autonomous behavior - periodic user engagement and goal adjustment
@fitagent_coach.on_interval(period=3600.0)  # Every hour
async def autonomous_user_engagement(ctx: Context):
    """Enhanced autonomous agent behavior - proactive engagement and goal adjustment"""
    
    try:
        ctx.logger.info("Running autonomous user engagement and goal adjustment")
        
        # Get all user profiles from enhanced client
        import sqlite3
        conn = sqlite3.connect(enhanced_venice_client.db_path)
        cursor = conn.cursor()
        ...
        for user_id in user_ids:
            try:
                # Perform autonomous goal adjustment
                adjustments = await enhanced_venice_client.autonomous_goal_adjustment(user_id)
                
                if adjustments["adjustments"]:
                    ctx.logger.info(f"Made {len(adjustments['adjustments'])} goal adjustments for user {user_id}")
                    
                    # Generate proactive message about adjustments
                    adjustment_summary = "\n".join([
                        f"- {adj['goal_type']}: {adj['old_target']} → {adj['new_target']} ({adj['reason']})"
                        for adj in adjustments["adjustments"]
                    ])
                    
                    proactive_response = await enhanced_venice_client.personalized_coaching_response(
                        user_id=user_id,
                        query=f"Goal adjustments made: {adjustment_summary}"
                    )
                    
                    ctx.logger.info(f"Sent autonomous goal adjustment notification to user {user_id}")
                
                # Analyze behavior patterns for insights
                insights = await enhanced_venice_client.get_behavioral_insights(user_id)
                
                # Check for users needing proactive engagement
                profile = enhanced_venice_client.get_user_profile(user_id)
                if profile["last_interaction"]:
                    last_time = datetime.fromisoformat(profile["last_interaction"])
                    ...
                    if hours_since_last >= 24:
                        ...
                        proactive_response = await enhanced_venice_client.personalized_coaching_response(
                            user_id=user_id,
                            query=f"Proactive check-in after {hours_since_last:.1f} hours of inactivity"
                        )
                        
                        ctx.logger.info(f"Sent proactive engagement to user {user_id}")
                
            except Exception as user_error:
                ctx.logger.error(f"Error processing user {user_id}: {user_error}")
        
    except Exception as e:
        ctx.logger.error(f"Error in autonomous engagement: {e}")

# New autonomous behavior - behavioral change tracking
@fitagent_coach.on_interval(period=21600.0)  # Every 6 hours
async def autonomous_behavior_analysis(ctx: Context):
    """Autonomous behavioral change tracking for ASI Alliance prize"""
    
    try:
        ctx.logger.info("Running autonomous behavior analysis")
        
        # Get all users for behavior analysis
        import sqlite3
        conn = sqlite3.connect(enhanced_venice_client.db_path)
        cursor = conn.cursor()
        
        cursor.execute('SELECT user_id FROM user_profiles WHERE total_interactions > 5')
        active_users = [row[0] for row in cursor.fetchall()]
        conn.close()
        
        behavior_summary = {
            "analysis_timestamp": datetime.now().isoformat(),
            "total_users_analyzed": len(active_users),
        ...
        for user_id in active_users:
            try:
                ...
                if insights["behavior_change_indicators"]:
                    behavior_summary["behavior_changes_detected"] += len(insights["behavior_change_indicators"])
                
                # Track goal improvements
                for trend in insights["goal_achievement_trends"]:
                    if trend["completion_rate"] > 80:
                        behavior_summary["goal_improvements"] += 1
                
                # Track coaching effectiveness
                effectiveness = insights["coaching_effectiveness"]
                if effectiveness["engagement_rate"] > 0.5:
                    behavior_summary["coaching_effectiveness"].append({
                        "user_id": user_id,
                        "engagement_rate": effectiveness["engagement_rate"],
                        "goal_completion": effectiveness["goal_completion_average"],
                        "behavior_strength": effectiveness["behavior_pattern_strength"]
                    })
                
                ctx.logger.info(f"Analyzed behavior patterns for user {user_id}")
                
            except Exception as user_error:
                ctx.logger.error(f"Error analyzing user {user_id}: {user_error}")
        
        # Log comprehensive behavior analysis results
        ctx.logger.info(f"Behavior Analysis Summary: {json.dumps(behavior_summary, indent=2)}")
        
        # Save behavior analysis for ASI Alliance prize documentation
        with open("behavior_analysis_log.json", "a") as f:
            f.write(json.dumps(behavior_summary) + "\n")
        
    except Exception as e:
        ctx.logger.error(f"Error in behavior analysis: {e}")
